...
// }
 ...
 if(gameState === PLAY){
  ...
  heart.y = balloon.y - 200;
  heart1.y = balloon.y - 200;
  heart2.y = balloon.y - 200;
...
// [index+1].x = balloon.x + 1
  ...
  stroke("blue");
  textSize(25);
  text("Score : " + score, balloon.x + 400, balloon.y - 250);
  // if (!lives===0){
  text("Use the up and down arrow keys to move the Hot Air Balloon!",balloon.x - 580, balloon.y - 250);
  // }
}
...
function readPosition(data){
  ...
  function spawnCoin(){
    ...
    if(frameCount % 10 === 0){
      ...
      coin.y = Math.round(random(-10000, 10000));
      coin.velocityX = -5;
